        ...
        <div className="space-y-8">
            <div className="flex flex-col space-y-4">
              ...
              { getGreetingLoading ? (
                ...
                <p className={!getGreetingError ? `text-lg text-center` : `text-lg text-center text-red-500`}>{ !getGreetingError ? greeting : `There was an error getting the greeting` }</p>
              )}
            </div>
            <div className="space-y-8">
              <div className="flex flex-col space-y-4">
                ...
                >
                  { !setGreetingLoading ? `Set your new greeting on the blockchain` : `Setting greeting...` }
                </button>
                { !address && <button className="text-sm text-gray-500 text-center underline hover:opacity-80" onClick={openConnectModal}>Connect your wallet to set a new greeting</button>}
                { address && !newGreeting && <p className="text-sm text-gray-500 text-center">Type something to set a new greeting</p>}
                { setGreetingError && <p className="text-sm text-red-500 text-center">There was an error setting your new greeting.</p>}
              </div>
            </div>
        </div>
    )
